...
const station_railway_data = JSON.parse(
  fs.readFileSync(station_railway_data_path)
);
const unknown_data = JSON.parse(fs.readFileSync(unknown_data_file_path));

const company_data = parse(fs.readFileSync(process.env.COMPANY_CSV_FILE))
  .filter((_, idx) => idx)
  .map((row) => ({
    companyCode: +row[0],
...
const railway_data = station_railway_data.railways
  .map((data) => ({
  ...
  .concat(
    unknown_data.railways.map((data) => ({
      railwayCode: data.railwayCode,
      railwayName: change_width(data.railwayName),
      formalName: change_width(data.railwayFormalName),
      railwayKana: kanaToHira(data.railwayKana),
...
const stationGroup_data = Object.keys(stationGroup_cnt).map(
  (stationGroupCode) => {
...
const station_data = station_railway_data.stations
  .map((data) => ({
  ...
  .concat(
    unknown_data.stations.map((data) => ({
      stationCode: data.stationCode,
      stationGroupCode: data.stationGroupCode,
      railwayCode: data.railwayCode,
      lat: data.lat,
...
const railPaths = new RailPaths(
  station_railway_data.nextStations,
  station_data
);
...
(async () => {
  ...
  db.transaction(() => {
    ...
    company_data.forEach((data) => {
      comp_stmt.run(data.companyCode, data.companyName, data.formalName);
    });
  ...
  `
    ...
    station_railway_data.nextStations.forEach((data) => {
      data.left.forEach((code) => {
        next_stmt.run(data.stationCode, code, 0);
      });
      data.right.forEach((code) => {
        next_stmt.run(data.stationCode, code, 1);
      });
    });
    ...
    railPaths.getRailwayList().map((railwayCode) => {
      railPaths.getPaths(railwayCode).map((path, pathId) => {
        path.map((cord, ord) =>
          path_stmt.run(railwayCode, pathId, ord, cord[0], cord[1])
        );
      });
    });
  })();
