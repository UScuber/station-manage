...
const encode_str = (s) =>
  btoa(String.fromCharCode.apply(null, new TextEncoder().encode(s)));
...
const arrange_stationGroupCode = (stationGroupCode, stationName) => {
  ...
  if (!(stationName in stationGroupCode_data[stationGroupCode])) {
    stationGroupCode_data[stationGroupCode][stationName] = Object.keys(
      stationGroupCode_data[stationGroupCode]
    ).length;
  }
  return parseInt(
    stationGroupCode + "" + stationGroupCode_data[stationGroupCode][stationName]
  );
};
...
const station_ekidata = parse(fs.readFileSync(process.env.STATION_CSV_FILE))
  ...
  .map((row) => ({
    ...
    prevCompanyName:
      company_data[railway_data[row[5]].companyCode].prevCompanyName,
    prefCode: +row[6],
    lng: +row[9],
    lat: +row[10],
  }));
...
let join_ekidata = {};
station_ekidata.forEach(
  (data) => (join_ekidata[data.stationCode] = { left: [], right: [] })
);
parse(fs.readFileSync(process.env.JOIN_CSV_FILE))
  .filter((_, idx) => idx)
  .forEach((row) => {
    const left = +row[1],
      right = +row[2];
...
let filtered_station_ekidata = station_ekidata.filter(
  (data) =>
    join_ekidata[data.stationCode].left.length +
    join_ekidata[data.stationCode].right.length
);

Object.keys(join_ekidata).forEach((stationCode) => {
  join_ekidata[stationCode].left = Array.from(
    new Set(join_ekidata[stationCode].left)
  );
  join_ekidata[stationCode].right = Array.from(
    new Set(join_ekidata[stationCode].right)
  );
});

const ekispert_station_data = JSON.parse(
  fs.readFileSync("./data/station.json")
).flatMap((data) =>
  data.railway.map((rail) => ({
...
let kokudo_route_data = JSON.parse(
  fs.readFileSync(process.env.N02_STATION_FILE)
).features.map((elem) => {
  delete elem.type;
...
kokudo_route_data = kokudo_route_data
  .filter((elem) => elem !== null)
  // 重心を求める
  .map((elem) => {
    const lat =
      elem.coordinates.reduce(
        (tot, pos) => tot + (pos[0][1] + pos[pos.length - 1][1]) * 0.5,
        0
      ) / elem.coordinates.length;
    const lng =
      elem.coordinates.reduce(
        (tot, pos) => tot + (pos[0][0] + pos[pos.length - 1][0]) * 0.5,
        0
      ) / elem.coordinates.length;
...
kokudo_route_data.forEach((elem, index) => {
  ...
  if (station_name in kokudo_station_group_map) {
    ...
    for (let i = 0; i < station_name_indices.length; i++) {
      let dist = distance(
        kokudo_route_data[index],
        kokudo_route_data[station_name_indices[i][0]]
      );
      if (isNaN(dist)) {
        dist = 0;
      }
      if (dist <= max_same_station_dist && !black_list.includes(station_name)) {
        kokudo_station_group_map[station_name].push([
          index,
          station_name_indices[i][1],
        ]);
...
buffer +=
  ekispert_station_data
    .map(
      (data) =>
        `${data.stationCode} ${data.stationGroupCode} ${encode_str(
          data.stationName
        )} ` +
    ...
    .map(
      (data) =>
        ...
        ["left", "right"]
          .map(
            (dir) =>
              join_ekidata[data.stationCode][dir].length +
              " " +
              join_ekidata[data.stationCode][dir].join(" ")
          )
          .join(" ")
    )
    .join("\n") + "\n";
  ...
  kokudo_route_data
    .map((data) => {
      ...
      return (
        ...
        `${railwayName_cnt[data.railwayName]} ${encode_str(
          data.railwayName
        )} ` +
        `${company_cnt[data.railwayCompany]} ${encode_str(
          data.railwayCompany
        )} ` +
        `${(+data.lat).toFixed(6)} ${(+data.lng).toFixed(6)}`
      );
...
(async () => {
          ...
          ["left", "right"]
            .map(
              (dir) =>
                data[dir].length +
                " " +
                data[dir].map((v) => v.stationCode).join(" ")
            )
            .join(" ")
      )
      .join("\n") + "\n";
  ...
  const railway_detail_data = JSON.parse(
    fs.readFileSync("./data/railway.json")
  );
  ...
  const station_detail_data = JSON.parse(
    fs.readFileSync("./data/station.json")
  );
  ...
  // 新幹線の情報を追加
  filtered_station_ekidata = filtered_station_ekidata.concat(
    result_json.shinkansen.stations.map((data) => {
      const sub_stationCode = find_station_pair(data.stationCode);
      if (!sub_stationCode) {
        console.error("shinkansen is not linked");
        process.exit(1);
      }
      const info = find_station_data(
        (+sub_stationCode).toString().substring(0, 5)
      );
      ...
      return {
        ...
        prevCompanyName:
          company_data[railway_data[data.railwayCode].companyCode]
            .prevCompanyName,
  ...
  const stations_db = Array.from(
    await Promise.all(
      filtered_station_ekidata.map(async (data) => {
        const sub_stationCode = find_station_pair(data.stationCode);
        let kana = "";
        if (!sub_stationCode) {
  ...
  const railways_db = Object.keys(railway_data)
    .map((railwayCode) => {
  ...
  fs.writeFileSync(
    "data/station-railway-data.json",
    JSON.stringify(
      {
  ...
  // unknown data
  // 読み仮名がわからなかったもの
  const unknown_station_data = name_unlinked_stations.map((data) => ({
    stationCode: data.stationCode,
    stationGroupCode: data.stationGroupCode,
    stationName: data.prevStationName,
    railwayCode: data.railwayCode,
  ...
  if (fs.existsSync(unknown_data_file_path)) {
    ...
    fs.writeFileSync(
      unknown_data_file_path,
      JSON.stringify(
        { stations: unknown_station_data, railways: unknown_railway_data },
        null,
        "  "
      )
    );
    console.log(
      `Please fill out null values (file: ${unknown_data_file_path})`
    );
  }
})();
