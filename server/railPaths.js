...
class CalcRailwayPath {
  ...
  get_graph_type(st, tree){
    ...
    if(dirs_count[1] == 2 && dirs_count[2] == station_num-2) return this.type.LinearList;
    if(dirs_count[1] == 0 && dirs_count[2] == station_num) return this.type.Circle;
    if(dirs_count[1] == 1 && dirs_count[3] == 1 && dirs_count[2] == station_num-2) return this.type.WithLoop;
...
class RailPaths {
  ...
  getPaths(railwayCode){
    ...
    return paths.map(path => path.map(pos => this.positions[this.railway_station[railwayCode][pos]]));
  }
};
